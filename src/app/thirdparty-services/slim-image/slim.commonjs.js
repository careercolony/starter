...
var FileHopper = function (label) {
  ...
  */
	...
	return function () {
		function FileHopper(label) {
			var element = arguments.length <= 0 || arguments[1] === undefined ? document.createElement('div') : arguments[0];
...
var SlimButtons = ['remove', 'edit', 'download', 'upload', 'add'];
...
var Slim = function () {
   ...
   */
		...
		value: function _init() {
			...
			var initialImageSrc = this._options.imageUrl;
			if (initialImageSrc) {
			...
			var resultHTML = '\n\t\t<div class="slim-result">\n\t\t\t<img class="in" style="opacity:0; width: 100%; height: 100%" ' + (initialImageSrc ? 'src="' + initialImageSrc + '"' : '') + '><img><img style="opacity:0">\n\t\t</div>';
			...
			if (this._isImageOnly()) {
				...
				// setup hopper
				this._imageHopper = new FileHopper(this._options.label);
				...
				// render area
				area.innerHTML = '\n\t\t\t\t' + SlimLoaderHTML + '\n\t\t\t\t' + SlimUploadStatusHTML + '\n\t\t\t\t<div class="slim-drip"><span><span></span></span></div>\n\t\t\t\t<div class="slim-status"><div class="slim-label"></div></div>\n\t\t\t\t' + resultHTML + '\n\t\t\t';
   ...
   */
		...
		value: function _hideResult(callback) {
			...
			intro['style'] = "opacity: 1";
			intro['src'] = this._options['defaultImage'];
		...
		key: '_doAdd',
		value: function _doAdd(callback) {
			this._element.querySelector('input[type=file]').click();
		}
	}, {
		key: '_doUpload',
